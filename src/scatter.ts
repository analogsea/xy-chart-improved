...
import { valuesToFills } from './valuesToFills';
...
function getNearestDataIdxByX(u: uPlot, seriesIdx: number): number | null {
  ...
  const series = u.data[seriesIdx] as unknown as FacetSeries | undefined;
  const xValues = series?.[0] as number[] | undefined;

  if (seriesIdx === 0 || cursorLeft == null || xValues == null || xValues.length === 0) {
    return null;
  }

  const cursorX = u.posToVal(cursorLeft, 'x');
